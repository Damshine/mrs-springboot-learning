package com.github.mrs.rate2.aspect;

import com.github.mrs.rate2.annotation.RateLimiter;
import com.github.mrs.rate2.enums.LimitType;
import com.github.mrs.rate2.exception.ServiceException;
import com.github.mrs.rate2.util.IpUtils;
...
public class RateLimiterAspect {
     ...
     *
     ...
     获取一个组合的 key，所谓的组合的 key，就是在注解的 key 属性基础上，
     再加上方法的完整路径，如果是 IP 模式的话，就再加上 IP 地址。以 IP 模式为例，
     最终生成的 key 类似这样：
     rate_limit:127.0.0.1-org.javaboy.ratelimiter.controller.HelloController-hello（如果不是 IP 模式，那么生成的 key 中就不包含 IP 地址）。
     将生成的 key 放到集合中。
